...
public class MainApp {
    ...
    public static void main(String[] args) {
        ...
        // Создание зависимостей для BinaryOperationsService
        DirectConverter directConverter = new DirectConverter(); // Для работы с прямым кодом
        InverseConverter inverseConverter = new InverseConverter(); // Для работы с обратным кодом
        ComplementConverter complementConverter = new ComplementConverter(); // Для работы с дополнительным кодом
        BinaryToDecimalConverter binaryToDecimalConverter = new BinaryToDecimalConverter(); // Для преобразования из двоичного в десятичное
        BinaryAdder binaryAdder = new BinaryAdder(); // Для выполнения операций сложения в двоичном коде
        BinarySubtractor binarySubtractor = new BinarySubtractor(); // Для выполнения операций вычитания в двоичном коде
        BinaryMultiplier binaryMultiplier = new BinaryMultiplier(directConverter); // Для умножения в двоичном коде
        BinaryDivider binaryDivider = new BinaryDivider(); // Для деления в двоичном коде
        IEEE754Converter ieee754Converter = new IEEE754Converter(); // Для операций с числами с плавающей запятой

        // Создание экземпляра BinaryOperationsService с передачей всех зависимостей
        ...
        while (true) {
            ...
            switch (choice) {
                ...
                case 4 -> {
                    ...
                    // Форматируем результат в 8-битный двоичный код
